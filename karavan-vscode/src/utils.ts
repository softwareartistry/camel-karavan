...
export function saveBlockList(key: string, value: string) {
  if (workspace.workspaceFolders) {
    const uriFolder: Uri = workspace.workspaceFolders[0].uri;
    const blockingComponentsPath: string | undefined = workspace
      .getConfiguration()
      .get("Karavan.blockingComponentsPath");
    if (blockingComponentsPath && blockingComponentsPath.trim().length > 0) {
      const name = key + "s-blocklist.txt";
      write(
        path.join(uriFolder.path, blockingComponentsPath + "/" + name),
        value
      );
    } else {
...
export async function readKamelets(context: ExtensionContext) {
  ...
  if (kameletsPath && kameletsPath.trim().length > 0) {
    ...
    if (customKamelets && customKamelets.length > 0)
      yamls.push(...customKamelets);
  }
  return yamls;
}

async function readBuildInKamelets(context: ExtensionContext) {
  const kameletsPath = path.join(
  ...
  code
    .split("\n---\n")
    .map((c) => c.trim())
    .forEach((z) => result.push(z));
  return result;
}

export async function readSupportedComponents() {
  const supportedComponentsPath: string | undefined = workspace
    .getConfiguration()
    .get("Karavan.supportedComponents");
  if (supportedComponentsPath && supportedComponentsPath.trim().length > 0) {
    const filename = path.isAbsolute(supportedComponentsPath)
      ? supportedComponentsPath
      : path.resolve(supportedComponentsPath);
...
async function readFilesInDirByExtension(
  dir: string,
  extension: string
...
export async function readPropertyPlaceholders(context: ExtensionContext) {
  ...
  lines
    .filter(
      (line) =>
        !line.startsWith("camel.") &&
        !line.startsWith("jkube.") &&
        !line.startsWith("jib.")
    )
...
function getBeans(integration: Integration): BeanFactoryDefinition[] {
  ...
  const beans = integration.spec.flows?.filter(
    (e: any) => e.dslName === "Beans"
  );
...
export async function readTemplates(context: ExtensionContext) {
  ...
  const files = await readFilesInDirByExtension(
    path.join(context.extensionPath, "snippets"),
    "java"
  );
  files.forEach((v, k) => {
...
export async function readBlockTemplates(context: ExtensionContext) {
  const result = new Map<string, string>();
  const blockedListDir: string | undefined = workspace
    .getConfiguration()
    .get("Karavan.blockingComponentsPath");
...
export function nameFromTitle(
  type: "crd" | "plain" | "kamelet",
  title: string,
  kameletType?: KameletTypes
): string {
...
export function fileNameFromName(
  type: "crd" | "plain" | "kamelet",
  name: string,
  kameletType?: KameletTypes
): string {
  if (type === "kamelet") {
    ...
    return name.toLocaleLowerCase().endsWith(".camel.yaml")
      ? name
      : name.split(".")[0] + ".camel.yaml";
  }
}
...
export async function getCamelYamlFiles(baseDir: string) {
  const result: string[] = [];
  (await getAllFiles(baseDir, []))
    .filter((f) => f.endsWith(".camel.yaml"))
    .forEach((f) => {
      result.push(f);
    });
  return result;
...
export async function readCamelYamlFiles(dir: string) {
  ...
  const camelFiles = exportFolder
    ? files.filter((f) => !f.startsWith(fullExportFolder))
    : files;
...
export async function hasApplicationProperties(baseDir: string) {
  return (await getPropertyFiles(baseDir)).includes(
    baseDir + path.sep + "application.properties"
  );
}
  ...
  try {
    ...
    if (rootPath) {
      const readData = await readFile(
        path.resolve(rootPath, "application.properties")
      );
...
export async function write(fullPath: string, code: string) {
  const uriFile: Uri = Uri.file(fullPath);
  workspace.fs.writeFile(uriFile, Buffer.from(code, "utf8")).then(
    (value) => {},
...
export async function createApplicationProperties(
  runtime: string,
  gav: string,
  target: string
) {
  if (workspace.workspaceFolders) {
    ...
    const appPropertyName = "Karavan."
      .concat(runtimePrefix)
      .concat("ApplicationProperties");
    const targetPropertyName = "Karavan."
      .concat(runtimePrefix)
      .concat(capitalize(target))
      .concat("Properties");
...
function setMinikubeEnvVariables(env: string): Map<string, string> {
  ...
  const vars = linesAll
    .filter((l) => l !== undefined && l.startsWith("export"))
...
export async function getFileWithIntegnalConsumer(
  fullPath: string,
  uri: string,
  name: string
) {
  try {
    ...
    const route = TopologyUtils.findTopologyRouteNodes(integrations)
      .filter((t) => t?.from?.uri === uri && t?.from?.parameters?.name === name)
      .at(0);
    if (route) {
...
export async function getFileWithInternalProducer(
  fullPath: string,
  routeId: string
) {
  try {
