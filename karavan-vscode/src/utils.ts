...
export function save(relativePath: string, text: string) {
	if (workspace.workspaceFolders) {
		console.log('Saving file inside integration ' + workspace.workspaceFolders[0]);
		const uriFolder: Uri = workspace.workspaceFolders[0].uri;
		console.log('Saving file inside uri ' + uriFolder);
...
export function toCliFilename(filename: string): string {
	return /\s/.test(filename) ? '"' + filename + '"' : filename.replace(/\s/g, '\\ ');
}
...
export function getRouteId(filePath: string) {
	const fileName = _.last(_.split(filePath, path.sep));
	return _.replace(fileName, '.camel.yaml', '');
}
