...
export function activate(context: ExtensionContext) {
	...
	const applicationCommand = commands.registerCommand('karavan.create-application', async (...args: any[]) => {
		console.log('application creation triggered');
		if (rootPath) {
			...
			if (createApp) {
				window
					.showQuickPick(runtimeOptions, { title: 'Select Runtime', canPickMany: false })
					.then((runtime) => {
						window
							.showQuickPick(deployOptions, { title: 'Select Deploy Target', canPickMany: false })
							.then((target) => {
	...
	const deployCommand = commands.registerCommand('karavan.deploy', (...args: any[]) => {
		console.log('deploy command triggered', ...args);
		designer.deployRoute(args[0].path);
		// exec.camelDeploy(path);
	});
	...
	const generateRest = commands.registerCommand('karavan.generate-rest', async (...args: any[]) => {
		...
		window
			.showQuickPick(generateOptions, { title: 'Select REST Generator options', canPickMany: false })
			.then((value) => {
				switch (value) {
					case generateOptions[0]:
						inputFileName(rootPath, openApi);
						break;
					case generateOptions[1]:
						selectFileName(rootPath, openApi);
						break;
				}
			});
	});
	...
	commands.registerCommand('karavan.reportIssue', () => {
		commands.executeCommand(
			'open',
			Uri.parse(
				'https://github.com/apache/camel-karavan/issues/new?title=[VS+Code]New+report&template=issue_template.md',
			),
		);
	});
}
