...
export class DesignerView {
	...
	getFilenameFromWebView() {
		const filename = Array.from(KARAVAN_PANELS.entries())
			.filter(({ 1: v }) => v.active)
			.map(([k]) => k)[0];
	...
	inputIntegrationName(type: 'crd' | 'plain' | 'kamelet', rootPath?: string, kameletType?: KameletTypes) {
		window
			...
			.then((value) => {
				if (value) {
					console.log('integration file name', value, type, kameletType);
					...
					const relativePath =
						(this.rootPath ? rootPath?.replace(this.rootPath, '') : rootPath) + path.sep + filename;
					const fullPath = (rootPath ? rootPath : this.rootPath) + path.sep + filename;
					console.log('fullpath inside integration', fullPath);
					console.log('relative inside integration', relativePath);
					console.log('yaml inside integration', yaml);
					console.log('type inside integration', type);
					console.log('filename inside integration', filename);
					console.log('calling file save inside create integration', filename, fullPath, relativePath, yaml);
					utils.save(relativePath, yaml);
					this.openKaravanWebView(filename, relativePath, fullPath, yaml);
	...
	openKaravanWebView(filename: string, relativePath: string, fullPath: string, yaml?: string, tab?: string) {
		if (!KARAVAN_PANELS.has(relativePath)) {
			// Karavan webview
			const panel = window.createWebviewPanel('karavan', filename, ViewColumn.One, {
				enableScripts: true,
			...
			panel.iconPath = Uri.joinPath(this.context.extensionUri, 'icons/karavan.svg');
			...
			panel.webview.onDidReceiveMessage(
				(message) => {
					switch (message.command) {
						case 'save':
							console.log(
								'calling file save inside open karavan webview',
								relativePath,
	...
	sendIntegrationData(
		...
		utils
			.readCamelYamlFiles(path.dirname(fullPath))
			.then((files) => {
	...
	deployRoute(filePath: string) {
		const routeId = utils.getRouteId(filePath);
		if (routeId) {
			const filename = this.getFilenameFromWebView();
			if (filename && KARAVAN_PANELS.has(filename)) {
				const panel = KARAVAN_PANELS.get(filename);
				console.log('i found panel', panel);
				console.log('fileName', filename);
				panel?.webview.postMessage({ command: 'deployRoute', routeId });
			}
		}
	}
	...
	internalConsumerClick(panel: WebviewPanel, fullPath: string, uri?: string, name?: string, routeId?: string) {
		...
		if (uri && name) {
			utils
				.getFileWithIntegnalConsumer(fullPath, uri, name)
				.then((filename) => {
			...
			utils
				.getFileWithInternalProducer(fullPath, routeId)
				.then((filename) => {
					if (filename !== undefined) {
						commands.executeCommand('karavan.open', { fsPath: filename });
					}
