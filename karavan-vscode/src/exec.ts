...
import api from '../webview/client/RequestClient';
...
export function camelDeploy(filePath: string) {
	const routeId = utils.getRouteId(filePath);
	async function deployRoute() {
		try {
			const response = await api.RouteControllerApi.start(routeId);
			return response;
		} catch (err) {
	...
	// 		console.log('export folder', exportFolder);
	...
	// 				.get(
	// 					'Karavan.'
	// 						.concat(runtime.replaceAll('-', ''))
	// 						.concat(utils.capitalize(target))
	// 						.concat('Deploy'),
	// 				) || '';
	// 		const command = jbang
	// 			.createExportCommand(filePath, runtime)
	// 			.concat(' && ')
	// 			.concat(deployCommand)
	// 			.concat(' -f ')
	// 			.concat(exportFolder);
