...
import {
	zsegmentAtlasComponent,
	zsegmentTransformerComponent,
	zsegmentSourceComponent,
	zsegmentDestinationComponents,
...
class App extends React.Component<Props, State> {
		...
		switch (message.command) {
			...
			case 'components':
				Promise.all([
					...
					.then(([zsegmentTransformer, zsegmentAtlas, zsegmentSources, zsegmentDestinations]) => {
						zsegmentSources.forEach((source) => {
							message.components.push(JSON.stringify(source));
						});
						zsegmentDestinations.forEach((destination) => {
							message.components.push(JSON.stringify(destination));
						});
						message.components.push(zsegmentTransformer, zsegmentAtlas);
			...
			case 'blockList':
				...
				const blockListMap = new Map(Object.keys(blockList).map((key) => [key, blockList[key]])).forEach(
					(list, key) => {
	...
	saveJavCode(name: string, code: string) {
		...
		vscode.postMessage({
			command: 'saveCode',
			name: name,
			yamlFullPath: this.state.fullPath,
			yamFileName: this.state.filename,
			code: code,
		});
	}
	...
	savePropertyPlaceholder(key: string, value: string) {
		vscode.postMessage({
			command: 'savePropertyPlaceholder',
			key: key,
			value: value,
		});
	}
	...
	public render() {
		...
		return (
			<AppContext.Provider value={{ filename, currentInterface }}>
						...
						<KaravanDesigner
							...
							onGetCustomCode={(name, javaType) => {
								...
								if (code === undefined || code.length === 0)
									code = TemplateApi.generateCode(javaType, name);
