...
export function ComponentPropertyField(props: Props) {
  ...
  const [integration, files] = useIntegrationStore(
    (state) => [state.integration, state.files],
    shallow
  );
  const [dark, setSelectedStep, beans] = useDesignerStore(
    (s) => [s.dark, s.setSelectedStep, s.beans],
    shallow
  );

  const [selectStatus, setSelectStatus] = useState<Map<string, boolean>>(
  ...
  function isSelectOpen(propertyName: string): boolean {
    return (
      selectStatus.has(propertyName) && selectStatus.get(propertyName) === true
    );
  }
  ...
  function getSelectBean(property: ComponentProperty, value: any) {
    ...
    if (beans) {
      selectOptions.push(
        <SelectOption key={0} value={"Select..."} isPlaceholder />
      );
      selectOptions.push(
        ...beans.map((bean) => (
          <SelectOption
  ...
  function getInternalUriSelect(property: ComponentProperty, value: any) {
    ...
    const componentName = getInternalComponentName(
      property.name,
      props.element
    );
    const internalUris = CamelUi.getInternalRouteUris(
      integration,
      componentName,
      false
    );
    let uris: string[] = CamelUi.getInternalUris(
      files,
      checkUri("direct"),
      checkUri("seda"),
      checkUri("vertx")
    ...
    uris = [
      ...new Set(
        uris.map((e) => (e.includes(":") ? e.split(":")?.at(1) || "" : e))
      ),
    ];
              ...
              onClick={(e) => {
                if (value) {
                  const newRoute = !internalUris.includes(value.toString())
                    ? CamelUi.createNewInternalRoute(
                        componentName.concat(...":", value.toString())
                      )
                    : undefined;
                  parametersChanged(
                    property.name,
      ...
      <InputGroup>
        {inInfrastructure && !showEditor && !noInfraSelectorButton && (
          <Tooltip
            position="bottom-end"
            content={
              "Select from " + capitalize(InfrastructureAPI.infrastructure)
            }
          >
            <Button
              variant="control"
              onClick={(e) => openInfrastructureSelector(property.name)}
            >
              {icon}
            </Button>
          </Tooltip>
  ...
  function getSelect(property: ComponentProperty, value: any) {
    ...
    if (property.enum && property.enum.length > 0) {
      ...
      property.enum.forEach((v) =>
        selectOptions.push(<SelectOption key={v} value={v} />)
      );
    }
    return (
      <Select
        ...
        selections={
          value !== undefined ? value.toString() : property.defaultValue
        }
        isOpen={isSelectOpen(property.name)}
        aria-labelledby={property.name}
        direction={SelectDirection.down}
  ...
  function getSwitch(property: ComponentProperty) {
    const isValueBoolean =
      textValue?.toString() === "true" || textValue?.toString() === "false";
    const isDisabled =
    ...
    const isChecked =
      textValue !== undefined
        ...
        : property.defaultValue !== undefined &&
          ["true", true].includes(property.defaultValue);
    return (
  ...
  function getLabel(property: ComponentProperty, value: any) {
    const labelClassName = PropertyUtil.hasComponentPropertyValueChanged(
      property,
      value
    )
      ? "value-changed"
