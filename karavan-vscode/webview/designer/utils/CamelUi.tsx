...
export class CamelUi {
	...
	static getSelectorModelsForParent = (parentDsl: string | undefined, showSteps: boolean = true): DslMetaModel[] => {
		...
		if (!parentDsl) {
			...
			result.push(...CamelUi.getSourceDslMetaModel('consumer'));
		} else {
			if (showSteps) {
				if (
					parentDsl &&
				...
				result.push(...CamelUi.getTargetDslMetaModel('producer'));
			} else {
	...
	static getComponentsDslMetaModel = (type: 'consumer' | 'producer'): DslMetaModel[] => {
		return ComponentApi.getComponents()
			.filter((c) => c.component.type === 'COMPONENT' || !c.component.type)
	...
	static getSourceDslMetaModel = (type: 'consumer' | 'producer'): DslMetaModel[] => {
		return ComponentApi.getComponents()

			.filter((c) => c.component.type === 'SOURCE')
			.filter((c) => (type === 'consumer' ? !c.component.producerOnly : !c.component.consumerOnly))
			.map(
				(c) =>
					new DslMetaModel({
						dsl: type === 'consumer' ? 'FromDefinition' : 'ToDefinition',
						uri: 'zsegment',
						navigation: 'interface Source',
						type: type === 'consumer' ? 'consumer' : 'producer',
						title: c.component.title,
						description: c.component.description,
						properties: c.component.parameters,
					}),
			);
	};

	static getTargetDslMetaModel = (type: 'consumer' | 'producer'): DslMetaModel[] => {
		return ComponentApi.getComponents()
			.filter((c) => c.component.type === 'TARGET')
			.filter((c) => (type === 'consumer' ? !c.component.producerOnly : !c.component.consumerOnly))
			.map(
				(c) =>
					new DslMetaModel({
	...
	static getElementTitle = (element: any): string => {
		if (element.dslName === 'RouteDefinition') {
			...
			['ToDefinition', 'ToDynamicDefinition', 'FromDefinition', 'KameletDefinition'].includes(element.dslName) &&
			(element as any).uri
		) {
			const uri = (element as any).uri;
			const kameletTitle =
				uri && uri.startsWith('kamelet:') ? KameletApi.findKameletByUri(uri)?.title() : undefined;
			const sourceTargetTitle =
				uri && uri.startsWith('zsegment')
					? CamelUtil.capitalizeName(element?.parameters?.name ?? element?.parameters?.name ?? '')
					: undefined;

			if (kameletTitle) {
				return kameletTitle;
			} else if (sourceTargetTitle) {
				return sourceTargetTitle;
			}
