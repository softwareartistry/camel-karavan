...
import { VariableUtil } from "core/api/VariableUtil";
...
export function KaravanDesigner(props: Props) {
  ...
  useEffect(() => {
    const sub = EventBus.onIntegrationUpdate()?.subscribe(
    ...
    try {
      ...
      InfrastructureAPI.setOnSavePropertyPlaceholder(
        props.onSavePropertyPlaceholder
      );
      InfrastructureAPI.setOnInternalConsumerClick(
        props.onInternalConsumerClick
      );
  ...
  function makeIntegration(yaml: string, filename: string): Integration {
    try {
      ...
      if (yaml && type !== "none") {
        const i = CamelDefinitionYaml.yamlToIntegration(
          props.filename,
          props.yaml
        );
        return i;
  ...
  function getTab(
    ...
    const count =
      counts.has(icon) && counts.get(icon) ? counts.get(icon) : undefined;
    const showCount = title !== "Route" && count && count > 0;
      ...
      <div className="top-menu-item" style={{ color: color }}>
        ...
        {showBadge && badge && (
          <Button
            ...
            onClick={(event) =>
              EventBus.sendAlert(message[0], message[1], "danger")
            }
          />
        )}
      </div>
  ...
  return (
    <PageSection
      variant={
        props.dark ? PageSectionVariants.darker : PageSectionVariants.light
      }
      className="page"
      isFilled
      ...
      <div className={"main-tabs-wrapper"}>
        ...
        >
          {isKamelet && (
            <Tab
              eventKey="kamelet"
              title={getTab("Definitions", "Kamelet Definitions", "kamelet")}
            ></Tab>
          )}
          <Tab
            eventKey="routes"
            title={getTab("Routes", "Integration flows", "routes")}
          ></Tab>
          {!isKamelet && (
            <Tab
              eventKey="rest"
              title={getTab("REST", "REST services", "rest")}
            ></Tab>
          )}
          <Tab
            eventKey="beans"
            title={getTab("Beans", "Beans Configuration", "beans")}
          ></Tab>
          {props.showCodeTab && (
            <Tab
              eventKey="code"
              title={getTab("YAML", "YAML Code", "code", true)}
            ></Tab>
          )}
        </Tabs>
      </div>
