  ...
  (set, get) => ({
    ...
    getVariables: () => {
      ...
      const otherFiles = files.filter(
        (file) => file.name !== integration.metadata.name
      );
      const currentVariables = VariableUtil.findVariablesInIntegrations([
        integration,
      ]);
...
export const useDesignerStore = createWithEqualityFn<
  DesignerState & DesignerAction
>(
  (set) => ({
    ...designerState,
