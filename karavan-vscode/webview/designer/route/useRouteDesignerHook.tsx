...
export function useRouteDesignerHook() {
  ...
  function onCommand(
    command: Command,
    printerRef: React.MutableRefObject<HTMLDivElement | null>
  ) {
  ...
  const selectElement = (element: CamelElement) => {
    ...
    if (shiftKeyPressed) {
      const hasFrom =
        uuids
          .map(
            (e) =>
              CamelDefinitionApiExt.findElementInIntegration(integration, e)
                ?.dslName === "FromDefinition"
          )
          .filter((r) => r).length > 0;
      canNotAdd =
        hasFrom || (uuids.length > 0 && element.dslName === "FromDefinition");
    }
    ...
    const i = CamelDisplayUtil.setIntegrationVisibility(
      integration,
      element.uuid
    );
    ...
    const uuid: string = uuids.includes(element.uuid)
      ? element.uuid
      : uuids.at(0) || "";
  ...
  function copyToClipboard(): void {
    ...
    selectedUuids.forEach((selectedUuid) => {
      const selectedElement = CamelDefinitionApiExt.findElementInIntegration(
        integration,
        selectedUuid
      );
      if (selectedElement) {
  ...
  function pasteFromClipboard(): void {
    if (
      ...
      clipboardSteps.length === 1 &&
      clipboardSteps[0]?.dslName === "RouteDefinition"
    ) {
      const clone = CamelUtil.cloneStep(clipboardSteps[0], true);
      addStep(clone, "", 0);
    } else if (selectedUuids.length === 1) {
      const targetMeta = CamelDefinitionApiExt.findElementMetaInIntegration(
        integration,
        selectedUuids[0]
      );
  ...
  const openSelector = (
    ...
    setSelectorTabIndex(
      parentId === undefined && parentDsl === undefined ? "components" : "eip"
    );
  };
        ...
        if (routeId !== undefined) {
          ...
          const route = CamelDefinitionApi.createRouteDefinition({
            from: new FromDefinition({
              uri: dsl.uri,
              parameters: dsl.uri === "zsegment" ? dsl.properties : undefined,
            }),
            nodePrefixId: nodePrefixId,
          });
          addStep(route, parentId, position, group, id);
        }
        break;
      case "ToDefinition":
        ...
        const to = CamelDefinitionApi.createStep(dsl.dsl, {
          uri: dsl.uri,
          parameters: dsl.uri === "zsegment" ? dsl.properties : undefined,
        });
  ...
  function setDslDefaults(step: CamelElement): CamelElement {
    ...
    if (step.dslName === "ChoiceDefinition") {
      (step as ChoiceDefinition).when?.push(
        CamelDefinitionApi.createStep("WhenDefinition", undefined)
      );
      (step as ChoiceDefinition).otherwise = CamelDefinitionApi.createStep(
        "OtherwiseDefinition",
        undefined
      );
    }
  ...
  const createRouteConfiguration = () => {
    ...
    const i = CamelDefinitionApiExt.addRouteConfigurationToIntegration(
      clone,
      routeConfiguration
    );
    setIntegration(i, false);
  ...
  const addStep = (
    ...
    const clone = CamelUtil.cloneIntegration(integration);
    const i = CamelDefinitionApiExt.addStepToIntegration(
      clone,
      step,
      parentId,
      position,
    ...
    const selectedStep =
      step.dslName === "RouteDefinition"
        ? (step as RouteDefinition).from
        : step;
    setIntegration(i, false);
  ...
  const replaceFrom = (dsl: DslMetaModel) => {
    ...
    if (selectedStep && fromId && dsl.uri) {
      ...
      const i = CamelDefinitionApiExt.replaceFromInIntegration(
        clone,
        fromId,
        newFrom
      );
  ...
  const moveElement = (source: string, target: string, asChild: boolean) => {
    const i = CamelDefinitionApiExt.moveRouteElement(
      integration,
      source,
      target,
      asChild
    ...
    const selectedStep = CamelDefinitionApiExt.findElementInIntegration(
      clone,
      source
    );
