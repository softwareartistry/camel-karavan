...
} from "core/api/CamelDefinitionApiExt";
...
export function DslElementHeader(props: Props) {
  ...
  function isHorizontal(): boolean {
    return ["MulticastDefinition", "LoadBalanceDefinition"].includes(
      props.step.dslName
    );
  }
  ...
  function getChildrenInfo(
    step: CamelElement
  ): [boolean, number, boolean, number, number] {
    const children = CamelDefinitionApiExt.getElementChildrenDefinition(
      step.dslName
    );
    const hasStepsField =
      children.filter((c: ChildElement) => c.name === "steps").length === 1;
    ...
    const hasNonStepsFields =
      children.filter(
        (c) =>
          c.name !== "steps" && c.name !== "expression" && c.name !== "onWhen"
      ).length > 0;
    ...
    return [
      ...
      stepsChildrenCount,
      hasNonStepsFields,
      nonStepChildrenCount,
      childrenCount,
    ];
  }

  function getHasWideChildrenElement(
    childrenInfo: [boolean, number, boolean, number, number]
  ) {
    const [
      hasStepsField,
      stepsChildrenCount,
    ...
    if (props.step.dslName === "SetHeadersDefinition") return false;
      ...
      hasStepsField &&
      stepsChildrenCount > 0 &&
      hasNonStepsFields &&
      nonStepChildrenCount > 0
    )
      return true;
  ...
  function getHeaderIconClasses(): string {
    ...
    if (
      ["ToDefinition", "FromDefinition", "PollDefinition"].includes(
        step.dslName
      )
    ) {
  ...
  function getHeader() {
    ...
    const inRouteConfiguration =
      parent !== undefined && parent.dslName === "RouteConfigurationDefinition";
    const showAddButton =
      !["CatchDefinition", "RouteDefinition"].includes(step.dslName) &&
    ...
    return (
      <div
        className={"dsl-element " + headerClasses}
  ...
  function getHeaderText(step: CamelElement): string {
    if (
      isKamelet() &&
      step.dslName === "ToDefinition" &&
      (step as any).uri === "kamelet:sink"
    ) {
      return "Sink";
    } else if (
      isKamelet() &&
      step.dslName === "FromDefinition" &&
      (step as any).uri === "kamelet:source"
    ) {
      return "Source";
    } else {
      return (step as any).description
        ? (step as any).description
        : CamelUi.getElementTitle(props.step);
    }
  }

  function getHeaderTextWithTooltip(
    step: CamelElement,
    ...
    if (
      CamelDefinitionApiExt.hasElementWithId(integration, (step as any).id) > 1
    ) {
      checkRequired[0] = false;
      checkRequired[1].push("Id should be unique");
    }
    let className = hasWideChildrenElement
      ? "text text-right"
      : "text text-bottom";
  ...
  function getHeaderTooltip(): string | React.JSX.Element | undefined {
    ...
    if (CamelUi.isShowUriTooltip(props.step))
      return CamelUi.getUriTooltip(props.step);
    return undefined;
  }
  ...
  function getAddElementButton() {
    return (
      <Tooltip
        ...
        content={
          <div>
            {"Add DSL element to " + CamelDisplayUtil.getTitle(props.step)}
          </div>
        }
      >
        <button
