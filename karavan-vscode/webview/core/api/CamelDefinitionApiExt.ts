...
export class CamelDefinitionApiExt {
  ...
  private static getFlowsOfType(
    integration: Integration,
    type: string
  ): CamelElement[] {
    return (
      integration.spec.flows?.filter((flow) => flow.dslName === type) ?? []
    );
  }

  private static getFlowsNotOfTypes(
    integration: Integration,
    types: string[]
  ): any[] {
    return (
      integration.spec.flows?.filter((flow) => !types.includes(flow.dslName)) ??
      []
    );
  }

  static replaceFromInIntegration = (
    integration: Integration,
    fromId: string,
    newFrom: FromDefinition
  ): Integration => {
    const flows: any = [];
    CamelDefinitionApiExt.getFlowsNotOfTypes(integration, [
      "RouteDefinition",
    ]).forEach((bean) => flows.push(bean));
    CamelDefinitionApiExt.getFlowsOfType(integration, "RouteDefinition").map(
      (flow) => {
  ...
  static addStepToIntegration = (
    ...
    if (step.dslName === "RouteDefinition") {
      ...
      const routeConfigurations = CamelDefinitionApiExt.addStepToSteps(
        CamelDefinitionApiExt.getFlowsOfType(
          integration,
          "RouteConfigurationDefinition"
        ),
        step,
  ...
  static addStepToStep = (
    ...
    const children = CamelDefinitionApiExt.getElementChildrenDefinition(
      result.dslName
    );
    let added = false;
    ...
    for (const child of children.filter((child) => child.name !== "steps") ??
      ...
      if (result.uuid === parentId) {
        ...
        if (child.multiple) {
          ...
          (result as any)[child.name] = CamelDefinitionApiExt.addStepToStep(
            fieldValue,
            stepAdded,
            parentId,
            position
    ...
    if (!added && steps && result.uuid === parentId) {
      ...
      (result as any).steps = CamelDefinitionApiExt.addStepToSteps(
        (result as any).steps,
        stepAdded,
        parentId,
        position
  ...
  static addStepToSteps = (
    ...
    for (const element of steps) {
      const newStep = CamelDefinitionApiExt.addStepToStep(
        element,
        step,
        parentId,
        position
  ...
  static findElementInIntegration = (
    integration: Integration,
    uuid: string
  ): CamelElement | undefined => {
    return CamelDefinitionApiExt.findElementMetaInIntegration(integration, uuid)
      ?.step;
  };

  static findElementMetaInIntegration = (
    integration: Integration,
    uuid: string
  ): CamelElementMeta => {
  ...
  static findElementPathUuids = (
    integration: Integration,
    uuid: string
  ): string[] => {
    const result: string[] = [];
    let meta = CamelDefinitionApiExt.findElementMetaInIntegration(
      integration,
      uuid
    );
    if (meta && meta.parentUuid) {
      while (meta.step?.dslName !== "FromDefinition") {
        if (meta.parentUuid) {
          ...
          meta = CamelDefinitionApiExt.findElementMetaInIntegration(
            integration,
            meta.parentUuid
          );
        } else {
  ...
  static findElementInElements = (
    ...
    uuid: string,
    result: CamelElementMeta = new CamelElementMeta(
      undefined,
      undefined,
      undefined
    ),
    ...
    if (steps !== undefined) {
      for (let index = 0, step: CamelElement; (step = steps[index]); index++) {
        if (step.uuid === uuid) {
          ...
          const ce = CamelDefinitionApiExt.getElementChildrenDefinition(
            step.dslName
          );
          for (const e of ce) {
            const cel = CamelDefinitionApiExt.getElementChildren(step, e);
            if (e.multiple) {
              result = CamelDefinitionApiExt.findElementInElements(
                cel,
                uuid,
                result,
                step.uuid
  ...
  static moveRouteElement = (
    ...
    const sourceFindStep = CamelDefinitionApiExt.findElementMetaInIntegration(
      integration,
      source
    );
    const sourceStep = sourceFindStep.step;
    const sourceUuid = sourceStep?.uuid;
    const targetFindStep = CamelDefinitionApiExt.findElementMetaInIntegration(
      integration,
      target
    );
    ...
    if (
      ...
      parentUuid &&
      sourceStep &&
      !CamelDefinitionApiExt.findElementPathUuids(integration, target).includes(
        source
      )
    ) {
  ...
  static deleteStepFromIntegration = (
    integration: Integration,
    uuidToDelete: string
  ): Integration => {
    const flows: any[] =
      integration.spec.flows?.filter(
        (flow) =>
          !["RouteConfigurationDefinition", "RouteDefinition"].includes(
            flow.dslName
          )
      ) ?? [];
  ...
  static deleteStepFromStep = (
    ...
    for (const e of ce) {
      const cel = CamelDefinitionApiExt.getElementChildren(step, e);
      if (e.multiple) {
        (result as any)[e.name] = CamelDefinitionApiExt.deleteStepFromSteps(
        ...
        if (prop?.hasOwnProperty("uuid")) {
          if (prop.uuid === uuidToDelete) {
            ...
            (result as any)[e.name] = CamelDefinitionApiExt.deleteStepFromStep(
              cel[0],
              uuidToDelete
            );
          }
    ...
    if (steps !== undefined) {
      for (const step of steps) {
        if (step.uuid !== uuidToDelete) {
          const newStep = CamelDefinitionApiExt.deleteStepFromStep(
            step,
            uuidToDelete
          );
          result.push(newStep);
  ...
  static addBeanToIntegration = (
    integration: Integration,
    bean: BeanFactoryDefinition
  ): Integration => {
    const flows: any[] = [];
    const beans: Beans[] =
      integration.spec.flows?.filter((flow) => flow.dslName === "Beans") ?? [];
    if (integration.spec.flows && beans.length === 0) {
      ...
      flows.push(
        ...(integration.spec.flows?.filter(
          (flow) => flow.dslName !== "Beans"
        ) ?? [])
      );
      for (const flow of beans) {
        const beans: BeanFactoryDefinition[] = [];
        if (
          (flow as Beans).beans.filter((b) => b.uuid === bean.uuid).length === 0
        ) {
          beans.push(
            ...(flow as Beans).beans.filter((b) => b.uuid !== bean.uuid)
          );
          beans.push(bean);
        } else {
  ...
  static deleteBeanFromIntegration = (
    integration: Integration,
    bean?: BeanFactoryDefinition
  ): Integration => {
    const flows: any[] = [];
  ...
  static deleteRouteConfigurationFromIntegration = (
    ...
    newFlows.push(
      ...flows.filter((flow) => flow.dslName !== "RouteConfigurationDefinition")
    );
    newFlows.push(
      ...flows.filter(
        (flow) =>
          flow.dslName === "RouteConfigurationDefinition" &&
          flow.uuid !== routeConfiguration.uuid
      )
    );
  ...
  static addRestMethodToIntegration = (
    ...
    const methodFunctions: {
      [key: string]: (rest: RestDefinition, method: CamelElement) => void;
    } = {
      GetDefinition: (rest: RestDefinition, method: CamelElement) => {
        rest.get = CamelDefinitionApiExt.addRestMethodToRestMethods(
          rest.get,
          method
        );
      },
      PostDefinition: (rest: RestDefinition, method: CamelElement) => {
        rest.post = CamelDefinitionApiExt.addRestMethodToRestMethods(
          rest.post,
          method
        );
      },
      PutDefinition: (rest: RestDefinition, method: CamelElement) => {
        rest.put = CamelDefinitionApiExt.addRestMethodToRestMethods(
          rest.put,
          method
        );
      },
      PatchDefinition: (rest: RestDefinition, method: CamelElement) => {
        rest.patch = CamelDefinitionApiExt.addRestMethodToRestMethods(
          rest.patch,
          method
        );
      },
      DeleteDefinition: (rest: RestDefinition, method: CamelElement) => {
        rest.delete = CamelDefinitionApiExt.addRestMethodToRestMethods(
          rest.delete,
          method
        );
      },
      HeadDefinition: (rest: RestDefinition, method: CamelElement) => {
        rest.head = CamelDefinitionApiExt.addRestMethodToRestMethods(
          rest.head,
          method
        );
      },
  ...
  static addRestMethodToRestMethods = (
    methods: CamelElement[] = [],
    method: CamelElement
  ): CamelElement[] => {
  ...
  static findRestMethodParent = (
    integration: Integration,
    method: CamelElement
  ): string | undefined => {
    const rests: RestDefinition[] =
      integration.spec.flows?.filter(
        (flow) => flow.dslName === "RestDefinition"
      ) ?? [];
  ...
  static deleteRestConfigurationFromIntegration = (
    integration: Integration
  ): Integration => {
    const flows: any[] = [];
  ...
  static deleteRestFromIntegration = (
    integration: Integration,
    restUuid?: string
  ): Integration => {
    const flows: any[] = [];
  ...
  static deleteRestMethodFromIntegration = (
    integration: Integration,
    methodUuid?: string
  ): Integration => {
    const flows: any[] = [];
    ...
    for (const flow of integration.spec.flows ?? []) {
      if (flow.dslName === "RestDefinition") {
        for (const method of methods) {
          if (flow[method]) {
            flow[method] = flow[method].filter(
              (item: any) => item.uuid !== methodUuid
            );
          }
        }
      }
  ...
  static getExpressionLanguageName = (
    ...
    if (expression) {
      for (const fieldName in expression) {
        ...
        const lang = Languages.find(
          (value: [string, string, string]) => value[0] === fieldName
        );
        if (lang) {
          const camelLangMetadata =
            CamelMetadataApi.getCamelLanguageMetadataByName(lang[0]);
  ...
  static getExpressionLanguageClassName = (
    expression: ExpressionDefinition | undefined
  ): string | undefined => {
    let result: string | undefined = undefined;
    if (expression) {
  ...
  static getExpressionValue = (
    expression: ExpressionDefinition | undefined
  ): CamelElement | undefined => {
    const language =
      CamelDefinitionApiExt.getExpressionLanguageName(expression);
    if (language) {
  ...
  static updateIntegrationRestElement = (
    integration: Integration,
    e: CamelElement
  ): Integration => {
    ...
    const isRest = (flow: any) =>
      flow.dslName === "RestDefinition" && flow.uuid === e.uuid;
    const isRestConfig = (flow: any) =>
      flow.dslName === "RestConfigurationDefinition" && flow.uuid === e.uuid;

    const isSingleRest = integration.spec.flows?.filter(isRest).length === 1;
    const isSingleRestConfig =
      integration.spec.flows?.filter(isRestConfig).length === 1;

    for (const flow of integration.spec.flows ?? []) {
      if (
        (isSingleRest && isRest(flow)) ||
        (isSingleRestConfig && isRestConfig(flow))
      ) {
  ...
  static updateIntegrationRouteElement = (
    integration: Integration,
    e: CamelElement
  ): Integration => {
    const elementClone = CamelUtil.cloneStep(e);
    ...
    for (const flow of integration.spec.flows ?? []) {
      if (flow.dslName === "RouteDefinition") {
        ...
        const routeConfiguration = CamelDefinitionApiExt.updateElement(
          flow,
          elementClone
        ) as RouteConfigurationDefinition;
        flows.push(
          CamelDefinitionApi.createRouteConfigurationDefinition(
            routeConfiguration
          )
        );
      } else {
  ...
  static updateIntegrationBeanElement = (
    integration: Integration,
    e: CamelElement
  ): Integration => {
    const elementClone = CamelUtil.cloneStep(e);
    ...
    if (className) {
      const properties =
        ...
        className.endsWith("Config")
          ...
          ? CamelMetadataApi.getCamelDataFormatMetadataByClassName(className)
              ?.properties
          : CamelMetadataApi.getCamelLanguageMetadataByClassName(className)
              ?.properties;

      if (properties) {
        for (const p of properties.filter(
          (p) => p.name !== "steps" && p.name !== "configurationRef"
        )) {
          switch (p.name) {
            case "uri":
              uri = p;
  ...
  static getElementChildrenDefinition = (dslName: string): ChildElement[] => {
    ...
    if (meta) {
      for (const property of meta.properties) {
        if (
          property.isObject &&
          CamelMetadataApi.getCamelModelMetadataByClassName(property.type)
        ) {
          result.push(
            new ChildElement(property.name, property.type, property.isArray)
          );
        }
      }
    }
