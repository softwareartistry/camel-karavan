...
export class ComponentApi {
  ...
  static saveSupportedComponents = (jsons: string): void => {
    ...
    const sc: SupportedComponent[] = (JSON.parse(jsons) as []).map(
      (json) => new SupportedComponent(json)
    );
  ...
  static saveComponents = (jsons: string[], clean: boolean = false): void => {
    ...
    const components: Component[] = jsons.map((json) =>
      ComponentApi.jsonToComponent(json)
    );
  ...
  static saveComponent = (json: string): void => {
    ...
    if (
      Components.findIndex(
        (c: Component) => c.component.name === component.component.name
      ) === -1
    ) {
  ...
  static getComponents = (): any[] => {
    ...
    if (SupportedOnly) {
      comps.push(
        ...Components.filter(
          (comp) =>
            SupportedComponents.findIndex(
              (sc) => sc.name === comp.component.name
            ) !== -1
        )
      );
    } else {
    ...
    return comps
      .map((comp) => {
        const sc = SupportedComponents.find(
          (sc) => sc.name === comp.component.name
        );
        if (sc !== undefined) {
  ...
  static findByName = (name: string): Component | undefined => {
    return ComponentApi.getComponents().find(
      (c: Component) => c.component.name === name
    );
  };
    ...
    if (name) {
      ...
      if (name === "salesforce") {
        ...
        if (parts.length === 2)
          ...
          result
            .set("operationName", parts.at(1) || "")
            .set("topicName", parts.at(2) || "");
      } else if (name === "cxf") {
        ...
        syntaxParts
          ...
          .forEach((part, index) => {
            if (index < syntaxParts.length - 1) {
              ...
              const start =
                newUri.indexOf(startSeparator) + startSeparator.length;
              const end = endSeparator
                ? newUri.indexOf(endSeparator, start)
                : newUri.length;
  ...
  static getComponentProperties = (
    ...
    const component: Component | undefined =
      ComponentApi.findByName(componentName);
    const properties: ComponentProperty[] = [];
    if (component !== undefined && component.properties) {
      for (const [key, value] of Object.entries(component.properties) as [
        string,
        any
      ][]) {
    ...
    result.push(
      ...properties.filter(
        (p) => p.label.length === 0 && p.kind !== "path" && !p.required
      )
    );
    result.push(
      ...properties.filter(
    ...
    result.push(
      ...properties.filter((p) => p.label === "formatting" && !p.required)
    );
    result.push(
      ...properties.filter(
        (p) =>
  ...
  static getComponentHeaders = (
    ...
    if (component !== undefined && component.headers) {
      for (const [key, value] of Object.entries(component.headers) as [
        string,
        any
      ][]) {
    ...
    return Array.from(
      new Map(properties.map((item) => [item.name, item])).values()
    );
  };
  ...
  static saveBlockedComponentName = (
    componentName: string,
    checked: boolean
  ) => {
