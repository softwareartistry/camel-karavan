...
export class CamelUtil {
  ...
  static cloneIntegration = (integration: Integration): Integration => {
    ...
    for (const flow of int.spec.flows || []) {
      if (flow.dslName !== "Beans") {
        ...
        newBeans.beans.push(
          ...(flow as Beans).beans.map((bean) => CamelUtil.cloneBean(bean))
        );
        flows.push(newBeans);
      }
    }

    for (const routeConfiguration of int.spec.flows?.filter(
      (flow) => flow.dslName === "RouteConfiguration"
    ) || []) {
      const newRouteConfiguration =
        CamelUtil.cloneRouteConfiguration(routeConfiguration);
  ...
  static camelizeObject = (body: any): any => {
    if (Array.isArray(body)) {
      return body.map((value) =>
        typeof value === "object" ? CamelUtil.camelizeObject(value) : value
      );
  ...
  static getKamelet = (element: CamelElement): KameletModel | undefined => {
    if (element.dslName === "KameletDefinition") {
      return KameletApi.findKameletByName(
        (element as KameletDefinition).name || ""
      );
    } else if (
      element.dslName === "ToDefinition" &&
      (element as ToDefinition).uri?.startsWith("kamelet:")
    ) {
      const kameletName = (element as ToDefinition).uri?.replace(
        "kamelet:",
        ""
      );
      return KameletApi.findKameletByName(kameletName);
    } else if (
      ["FromDefinition", "FromDefinition", "ToDefinition"].includes(
        element.dslName
      )
    ) {
      const uri: string = (element as any).uri;
  ...
  static getComponentProperties = (element: any): ComponentProperty[] => {
    ...
    const name =
      uri === "zsegment"
        ? element.parameters.name ?? element.parameters.name
  ...
  static checkRequired = (element: CamelElement): [boolean, string[]] => {
    ...
    if (elementMeta) {
      for (const property of elementMeta.properties.filter((p) => p.required)) {
        ...
        if (
          ...
          ["ExpressionSubElementDefinition", "ExpressionDefinition"].includes(
            property.type
          )
        ) {
          const expressionMeta =
            CamelMetadataApi.getCamelModelMetadataByClassName(
              "ExpressionDefinition"
            );
          const expressionCheck =
            expressionMeta &&
            value !== undefined &&
            expressionMeta?.properties.some((ep) => {
    ...
    if (className === "FromDefinition" || className === "ToDefinition") {
      if (!CamelUtil.isKameletComponent(element)) {
        const requiredProperties = CamelUtil.getComponentProperties(
          element
        ).filter((p) => p.required);
        for (const property of requiredProperties) {
          ...
          if (
            value === undefined ||
            (property.type === "string" && value.trim().length === 0)
          ) {
            result[0] = false;
            result[1].push(`${property.displayName} is required`);
          }
        }
        const secretProperties = CamelUtil.getComponentProperties(
          element
        ).filter((p) => p.secret);
        ...
        const filledParameters = elementAsAny
          ? Object.keys(elementAsAny.parameters)
          : [];
        const missingParameters =
          kamelet?.spec.definition.required?.filter(
            (name) => !filledParameters.includes(name)
          ) || [];
        if (missingParameters.length > 0) {
          allSet = false;
          result[1].push(
            ...missingParameters.map((name) => `${name} is required`)
          );
        }
  ...
  static checkIfKameletParameterSensitive(
    parameter: string,
    ...
    if (SensitiveKeys.includes(parameter)) {
      ...
      return (
        (kamelet?.spec.definition.properties?.[parameter] as any)?.type ===
        "password"
      );
    }
  }

  static findPlaceholdersInObject = (
    item: any,
  ...
  static createNewKameletCode = (
    kameletName: string,
    kameletType: KameletTypes,
    copyFromKameletName?: string
  ): string => {
    const integration = Integration.createNew(kameletName, "kamelet");
    const meta: MetadataLabels = new MetadataLabels({
      "camel.apache.org/kamelet.type": kameletType,
    });
    integration.metadata.labels = meta;
    if (copyFromKameletName !== undefined && copyFromKameletName !== "") {
      const kamelet = KameletApi.getKamelets()
        .filter((k) => k.metadata.name === copyFromKameletName)
        .at(0);
      if (kamelet) {
